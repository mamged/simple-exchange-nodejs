...
class OrderBook {
     ...
     */
    handleSnapshot(data, timestamp = Date.now()) {
     ...
     * @param {number} timestamp
     */
    handleDelta(deleteDelta = [], updateDelta = [], insertDelta = [], timestamp = Date.now()) {
        this.checkTimestamp(timestamp);
    ...
    checkTimestamp(timestamp) {
        ...
        if (this.lastUpdateTimestamp > timestamp) {
            throw new Error(`Received data older than last tick: ${{
    ...
    print() {
        var _a;
        // console.clear();
        console.log(`---------- ${this.symbol} ask:bid ${this.getBestAsk()}:${this.getBestBid()} & spread: ${(_a = this.getSpreadPercent()) === null || _a === void 0 ? void 0 : _a.toFixed(5)}%`);
